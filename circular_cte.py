...
class robot:
    ...
    def cte(self, radius):
        """
        calculates the cross track error using x and y coordinates and radius
        args:
            radius:
        """
        #left side of race course, 0 when exactly on circle: too far to left of radius
        if self.x < radius:
            cte = sqrt((self.x - radius) ** 2.0 + (self.y - radius) ** 2.0) - radius
        #right side of circle: too far to right of circle
        elif self.x > pi * radius:
            cte = sqrt((self.x - pi * radius) ** 2.0 + (self.y - radius) ** 2.0) - radius
        #2 x radius is height of radius: too far above radius 
        elif self.y > radius:
...
print('\nFinal parameters: {}->\t{}'.format(params, err))
