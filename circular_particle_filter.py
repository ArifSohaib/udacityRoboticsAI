...
class robot:
    ...
    def measurement_prob(self, measurements):
        ...
        for i in range(len(measurements)):
            ...
            # update Gaussian
            # if self.bearing_noise == 0.0:
            #     minimum_noise = 0.0000001
            #     error *= (exp(- (error_bearing ** 2) / (minimum_noise ** 2) / 2.0) /  
            #           sqrt(2.0 * pi * (minimum_noise ** 2)))
            # else:
    ...
    def move(self, motion,add_noise=False): 
        """
        args:
            motion[0]: steering angle in radians
            motion[1]: distance moved
        """
        if len(motion) != 2:
            raise ValueError("motion must be of size 2 motion: {}".format(motion))
        result = robot()
        ...
        distance = motion[1]
        if add_noise == True:
            steering = random.gauss(steering, result.steering_noise)
            distance = random.gauss(distance, result.distance_noise)
        ...
        if add_noise == True:
            result.set(x+distance_noise,y+distance_noise,orientation+steering_noise)
        else:
            result.set(x,y,orientation)
    ...
    def sense(self, add_noise=False): #do not change the name of this function
        ...
        for i in range(len(landmarks)):
            delta_x = landmarks[i][0] - self.y
            delta_y = landmarks[i][1] - self.x
            bearing = atan2(delta_x, delta_y)-self.orientation
            if add_noise==True:
